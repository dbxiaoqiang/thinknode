...
global.M = function (name) {
    ...
    let layer = arguments.length <= 2 || arguments[2] === undefined ? 'Model' : arguments[2];
...
global.T = function (name, http, data) {
    ...
    function runBehavior() {
        ...
        if (!behavior) {
            return _promise2.default.resolve(http.tagdata);
        }
        return _promise2.default.resolve(B(behavior, http, http.tagdata)).then(data => {
...
global.walkFilter = function (array, filter) {
    ...
    let walkArray = function walkArray(arr) {
        let rst = [];
    ...
    if (isObject(array)) {
        result = (0, _values2.default)(array);
        let keys = function keys(array) {
