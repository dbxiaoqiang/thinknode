...
//常用类的基类
['Cache', 'Behavior', 'Controller', 'Session', 'Model', 'Service', 'Logic'].forEach(function (item) {
 ...
 * 实例化模型
 * @param name 模型名称
 * @param layer 默认为Model,可选Logic
 * @param config 如果是字符串,值为db_prefix;如果是对象,值为数据库配置对象
 * @returns {*}
 * @constructor
 */
...
global.E = function (msg, isbreak) {
    ...
    return getPromise(new Error(msg), isbreak);
};
/**
 * 快速文件读取和写入
...
/**
 * 实例化模型基类
 * @param {[type]} name        [description] 表名,首字母大写
 * @param {[type]} config      [description] 如果是字符串,值为db_prefix;如果是对象,值为数据库配置对象
 */
