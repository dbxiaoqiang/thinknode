...
//常用类的基类
['Behavior', 'Controller', 'Model', 'Service', 'Logic', 'Cache', 'Session'].forEach(function (item) {
...
global.D = function (name, layer, config) {
    if (!isString(name)) {
        return thinkRequire(name.__filename)(name.modelName, config);
    }
    if (isEmpty(layer)) {
        layer = 'Model';
    }
...
global.M = function (config) {
    ...
    return thinkRequire(model)(undefined, config);
};
/**
 * 缓存的设置和读取
 * 获取返回的是一个promise
