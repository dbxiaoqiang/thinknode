...
global.thinkRequire = function (name) {
    ...
    var Cls = thinkCache(THINK.CACHES.ALIAS_EXPORT, name);
    if (!isEmpty(Cls)) {
        return Cls;
    }
...
global.C = function (name, value) {
    var _conf = thinkCache(THINK.CACHES.CONF);
    //获取所有的配置
    if (!name && !value) {
    ...
    try {
        ...
        if (value === undefined || value === '') {
            ...
            return instance.rm(name); //删除缓存
                ...
                return instance.set(name, (0, _stringify2.default)(value), options.timeout);
            }
    } catch (e) {
        return Err(e);
    }
...
global.parallelLimit = function (key, data, callback) {
    ...
    var Limit = thinkCache(THINK.CACHES.COLLECTION, 'limit');
    if (!Limit) {
