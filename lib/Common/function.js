...
global.thinkImport = function (name) {
    ...
    var load = function load(name, filepath) {
        return System['import'](filepath).then(function (obj) {
            if (isFunction(obj)) {
                obj.prototype.__filename = filepath;
            }
            if (obj) {
...
global.Class = function (superCls, prop) {
    var cls = function cls() {
        function T(args) {
            for (var _name in cls.__prop) {
                var val = cls.__prop[_name];
                if (isObject(val)) {
                    this[_name] = extend({}, val);
                } else if (isArray(val)) {
                    this[_name] = extend([], val);
                } else {
                    this[_name] = val;
    ...
    cls.extend = function (prop) {
        ...
        if (isObject(prop)) {
            for (var _name2 in prop) {
                var val = prop[_name2];
                if (isFunction(val)) {
                    this.prototype[_name2] = val;
                } else if (isObject(val)) {
                    cls.__prop[_name2] = extend({}, val);
                } else if (isArray(val)) {
                    cls.__prop[_name2] = extend([], val);
                } else {
                    cls.__prop[_name2] = val;
                }
            }
        }
        return this;
...
global.D = function (name, config) {
    var layer = arguments.length <= 2 || arguments[2] === undefined ? 'Model' : arguments[2];
...
global.O = function (http, msg, status) {
    ...
    var httpRes = function httpRes(http, msg) {
        if (!http.isend) {
            //控制台输出
