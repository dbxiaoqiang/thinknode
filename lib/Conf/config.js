...
module.exports = {
    ...
    filter_data: true, //Http参数安全过滤,建议开启,开启后参数值为数组的将被过滤
    //分组及控制器相关配置
    ...
    log_console_path: THINK.LOG_PATH, //日志文件存放路径
    ...
    session_type: '', //session存储类型, 空为内存，还可以为File
    ...
    //token配置
    token_on: true, //是否开启表单令牌功能,需要自行在控制器内调用
    ...
    db_ext_config: {safe: false}, //数据库连接时候额外的参数,safe安全模式下ORM不会实时映射修改数据库表,默认为false
    auto_close_db: false,  //自动关闭数据库连接
    db_safe: true, //ORM安全模式,如果为true
    //memcache配置
