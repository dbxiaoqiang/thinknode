...
exports.default = class extends _Base2.default {
    ...
    run() {
        ...
        return _promise2.default.resolve(this.bind()).then(() => {
    ...
    session(name, value, timeout) {
        ...
        return (0, _asyncToGenerator3.default)(function* () {
            _this.sessionStore(_this);
            if (!_this._session) {
                return null;
            }
            if (name === undefined) {
                return _this._session.rm();
            }
            try {
                if (value !== undefined) {
                    timeout = isNumber(timeout) ? timeout : C('session_timeout');
                    return _this._session.set(name, value, timeout);
                } else {
                    return _this._session.get(name);
                }
            } catch (e) {
                return null;
            }
     ...
     */
        ...
        return (0, _asyncToGenerator3.default)(function* () {
            yield _this2.write(obj, encoding);
            //this.emit('beforeEnd', this);
            _this2.isend = true;
            _this2.res.end();
            //this.emit('afterEnd', this);
            if (C('post_file_autoremove') && !isEmpty(_this2.file)) {
                let key,
                    path,
                    fn = function fn() {};
                for (key in _this2.file) {
                    path = _this2.file[key].path;
                    if (isFile(path)) {
                        _fs2.default.unlink(path, fn);
                    }
                }
            }
            return O(_this2, 200, '', _this2.isWebSocket ? 'SOCKET' : 'HTTP');
        })();
    }
