...
module.exports = Cache(function () {
    ...
    return {
        ...
        initData: function () {
            if (!this.promise) {
                var self = this;
                this.promise = model.table('Session').where({key: this.key}).find().then(function (data) {
                    if (isEmpty(data)) {
                        return model.table('Session').add({
                            key: self.key,
        ...
        flush: function () {
            ...
            return this.initData().then(function () {
                ...
                return model.table('Session').where({key: self.key}).update(data);
            });
        },
        /**
         * [gc description]
         ...
         */
        gc: function (now) {
            return model.table('Session').where({
                expire: {'<': now}
            }).delete();
        }
    };
