...
module.exports = Cache(function(){
	...
	return {
		...
		init: function(options){
			...
			this.cacheKey = this.namePrefix + options.cookie;
			this.data = {};
		},
		/**
		 * 初始化数据
		...
		initData: function(){
			if (!this.promise) {
				var self = this;
				this.promise = this.handle.get(this.cacheKey).then(function(value){
		...
		flush: function(){
			...
			return this.initData().then(function(){
				if (self.data === null) {
					return self.handle.delete(self.cacheKey);
				} else {
					return self.handle.set(self.cacheKey, JSON.stringify(self.data), self.options.timeout);
				}
			})
		}
	};
