...
App.listener = function (http) {
    ...
    if (C('use_proxy') && http.host !== http.hostname && !http.websocket) {
        if(THINK.APP_DEBUG){
            console.log(new Date().Format('yyyy-mm-dd hh:mi:ss') + ': ' + 403 + '   ' + http.req.url);
        }
...
App.exec = function (http) {
    ...
    if (!controller) {
        return E('Controller' + (http.controller ? ' `' + http.controller + '`' : '') + ' not found. ' +
...
App.execAction = function (controller, action, data, callMethod) {
    ...
    if (!isFunction(controller[act]) && !flag) {
        return E('action `' + action + '` not found.');
    }
...
App.sendError = function (http, error) {
    ...
    var errCode = C('error_code') || 500;
    if(THINK.APP_DEBUG){
        console.log(new Date().Format('yyyy-mm-dd hh:mi:ss') + ': ' + errCode + '   ' + http.req.url);
        console.log(message);
    }
    ...
    if(http.endProcess === 0){
        ...
        if (THINK.APP_DEBUG) {
            ...
            http.echo('<h1>( >﹏< )</h1>' +
                '<p>服务器君表示鸭梨很大,快快召唤攻城狮!!</p>').then(function () {
                http.end();
            });
        }
    }
