    ...
    return {
        ...
        initDb: function () {
            ...
            if (!dbInstances[configKey]) {
                ...
                dbInstances[configKey] = {client: client, connection: connection, schema: {}};
            }
        ...
        initModel: function (table) {
            ...
            return this.initDb().then(function (db) {
                if (db.client.collections && db.client.collections.hasOwnProperty(table)) {
                    ...
                    if (isEmpty(self.db.schema[table])) {
                        self.db.schema[table] = Waterline.Collection.extend({
                            identity: table,
                            tableName: table,
                            connection: 'default',
                            //migrate: 'safe',
                    ...
                    db.client.loadCollection(self.db.schema[table]);
                    promises = new Promise(function (fulfill, reject) {
                        db.client.initialize(db.connection, function (err, ontology) {
                            if (err) reject(err);
                            else fulfill(ontology);
                        });
                    });
                }
                return promises.then(function (schema) {
                    self.model = schema.collections[table];
                    return self.model;
                });
            });
        },
        ...
        getPk: function () {
            if (!isEmpty(this.attributes)) {
                for (var v in this.attributes) {
                    if (this.attributes[v].hasOwnProperty("primaryKey") && this.attributes[v].primaryKey === true) {
                        this.pk = v;
                    }
                }
            }
        ...
        parseOptions: function (oriOpts, extraOptions) {
            ...
            this._options = {};
            //获取表名
            var table = options.tableName = options.tableName || this.getTableName();
            //表前缀，Db里会使用
        ...
        add: function (data, options) {
            ...
            return this.parseOptions(options).then(function (options) {
                ...
                parsedData = data;
                return self.initModel(parsedOptions.tableName);
            }).then(function (model) {
            ...
            if (supportDb.toString().indexOf(this.config.db_type) > -1) {
                ...
                return this.parseOptions().then(function (options) {
                    if (model) {
                        ...
                        promises = self.initModel(options.tableName);
                    }
