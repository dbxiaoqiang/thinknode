...
var _default = (function (_base) {
    ...
    _default.prototype.initDb = function initDb() {
        ...
        if (!THINK.INSTANCES.DB[this.adapter]) {
            ...
            THINK.INSTANCES.DB[this.adapter] = new _Promise(function (fulfill, reject) {
                THINK.ORM.initialize(_this.dbOptions, function (err, ontology) {
    ...
    _default.prototype.setCollections = function setCollections(table) {
        var _this2 = this;
        ...
        if (!table || THINK.ORM.collections && isEmpty(THINK.ORM.collections[this.trueTableName])) {
            if (!isEmpty(this._relationLink)) {
                this._relationLink.forEach(function (rel) {
                    THINK.ORM.loadCollection(_this2.schema[rel.table]);
                });
    ...
    _default.prototype.setRelation = function setRelation(table, relation) {
        ...
        relation.forEach(function (rel) {
            if (!isEmpty(rel.type)) {
                switch (rel.type) {
                    case 2:
                        relationObj = _this3._getHasManyRelation(table, _this3.fields, rel);
                        break;
                    case 3:
                        relationObj = _this3._getManyToManyRelation(table, _this3.fields, rel);
                        break;
                    default:
                        relationObj = _this3._getHasOneRelation(table, _this3.fields, rel);
                        break;
                }
