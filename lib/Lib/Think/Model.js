...
var _default = (function (_base) {
        ...
        if (!THINK.INSTANCES.DB[this.configKey]) {
            (function () {
                var promise = undefined,
                    dbClient = new _waterline2['default']();
                if (dbClient.collections && dbClient.collections.hasOwnProperty(_this.trueTableName)) {
                    promise = getPromise(dbClient);
                } else {
                    if (!isEmpty(_this._relationLink)) {
                        _this._relationLink.forEach(function (rel) {
                            dbClient.loadCollection(this.schema[rel.table]);
                        });
                        dbClient.loadCollection(_this.schema[_this.trueTableName]);
                    } else {
                        if (isEmpty(_this.schema[_this.trueTableName])) {
                            _this.schema[_this.trueTableName] = _this.setSchema(_this.trueTableName, _this.fields);
                        }
                        dbClient.loadCollection(_this.schema[_this.trueTableName]);
                    }
                    promise = new _Promise(function (fulfill, reject) {
                        dbClient.initialize(_this.dbOptions, function (err, ontology) {
                            if (err) reject(err);else fulfill(ontology);
                        });
                    });
                }
                THINK.INSTANCES.DB[_this.configKey] = promise.then(function (client) {
                    _this.model = client.collections[_this.trueTableName];
                    return _this.model;
                });
            })();
        }
    ...
    _default.prototype.parseData = function parseData(data) {
        ...
        var result = _UtilValidJs2['default'](checkData);
        if (isEmpty(result)) {
            return data;
        }
        return E(result);
