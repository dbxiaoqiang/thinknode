...
var Model = module.exports = Class(function () {
    ...
    return {
        ...
        getTableName: function () {
            if (!this.trueTableName) {
                ...
                this.trueTableName = this.config.db_name + '.' + tableName.toLowerCase();
            }
        ...
        table: function (table, hasPrefix) {
            ...
            this._fields = {};
            this._options.table = hasPrefix ? this.config.db_name + '.' + table : this.config.db_name + '.' + this.tablePrefix + table;
            return this;
        },
        /**
         * 联合查询
     ...
     * })
                ...
                if (isObject(options.where) && !isEmpty(fields)) {
                    ...
                    for (var key in options.where) {
                        ...
                        if (fields.indexOf(key) > -1) {
                            ...
                            return E('field `' + key + '` in where condition is not valid');
                        }
                    }
                }
                //field反选
        ...
        _validData: function (data) {
            ...
            var err = json_message.slice(1, -1);
        ...
        add: function (data, options, replace) {
            ...
            return this.parseOptions(options).then(function (options) {
                ...
                var pk = self.getPk();
                parsedData[pk] = parsedData[pk] ? parsedData[pk] : self.db.getLastInsertId();
        ...
        update: function (data, options) {
            ...
            return this.parseOptions(options).then(function (options) {
                ...
                pk = pkey;
                data = self.parseData(parsedData);
        ...
        countSelect: function (options, pageFlag) {
            ...
            return this.parseOptions(options).then(function (options) {
                parsedOptions = options;
                if(options.alias && options.table.indexOf(options.alias) > -1){
                    options.table = options.table.split('AS')[0];
                }
                return self.options({
                    where: options.where,
                    cache: options.cache,
