...
THINK.require = function (name, type) {
    ...
    var Cls = THINK.loadCache(type || THINK.CACHES.ALIAS_EXPORT, name);
    ...
    try {
        var filepath = type ? THINK.loadCache(THINK.CACHES.ALIAS, type)[name] : THINK.loadCache(THINK.CACHES.ALIAS, name);
        if (filepath) {
...
THINK.cache = function (name, value, options) {
    try {
        ...
        var cls = THINK.adapter((THINK.config('cache_type') || 'File') + 'Cache');
...
THINK.config = function (name, value) {
    var _conf = THINK.loadCache(THINK.CACHES.CONF);
    //获取所有的配置
    if (!name && !value) {
...
THINK.data = function (name, value, rootPath) {
    try {
        ...
        if (THINK.isFile(filePath)) {
            var content = THINK.getFileContent(filePath);
            if (content) {
...
THINK.parallelLimit = function (key, data, callback) {
    ...
    var Limit = THINK.loadCache(THINK.CACHES.COLLECTION, 'limit');
    if (!Limit) {
...
THINK.run = function (name, http, data) {
    var list = THINK.HOOK[name] || [];
    var runItemMiddleware = function runItemMiddleware(list, index, http, data) {
        var item = list[index];
        if (!item) {
            return _promise2.default.resolve(data);
        }
        return THINK.use(item, http, data).then(function (result) {
            if (result === null) {
                return _promise2.default.resolve(data);
...
THINK.statusAction = function (http) {
    ...
    var _write = function _write(http, status, msg) {
        var content = '';
        var message = THINK.isError(msg) ? msg.message : msg;
    ...
    if (!http.isend && status > 399) {
        ...
        var tplFlag = THINK.config('tpl_custom_error');
        if (tplFlag === true) {
        ...
        if (tplFlag && http.loaded) {
            ...
            cls.assign('msg', msg);
            return cls.display(THINK.APP_PATH + '/Common/View/' + THINK.config('tpl_default_theme') + '/' + status + THINK.config('tpl_file_suffix')).catch(function (err) {
 ...
 */
(function () {
    for (var n in _objDefinePropertyNoWrite) {
